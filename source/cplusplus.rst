...
* 异常允许高水平的应用程序决定如何在嵌套函数内处理“不可能发生”的错误，而不像用错误记录代码那样模糊。
* 大多数的现代语言都使用异常机制。在 C++ 中使用异常使它与 Python、Java以及其它和 C++ 相近的语言保持一致。
* 一些第三方的 C++ 库使用了异常，如果不使用异常的话就很难和那些库集成。
* 异常是解决构造函数失败的唯一办法，虽然我们可以用一个工厂函数或者 ``Init()`` 函数去模拟构造函数，但是它们分别需要堆分配空间和新的非法状态。
* 在测试框架的时候，异常确实很好用。

缺点 ： 
'''''''''

* 如果你为一个函数加了 ``throw`` 语句，就必须检查所有的函数调用。要么它们至少有基本的异常安全保证，要么它们不会捕获异常安全退出。比如，如果 ``f()`` 函数调用了 ``g()``, ``g()`` 函数调用了 ``h()`` ，h 函数抛出了一个异常被 f 函数捕获了，g 就要小心，否则它就不能被完全清理。
* 一般来说，异常使得程序的控制流很难通过查看代码来确定：函数可能在你想不到的地方返回，这样使代码的维护和调试更加的困难。可以通过指定如何何地使用异常来最小化这样的开销，但是这样让开发者知道了解更多增加了负担。
* 异常安全需要 RAII 和不同的编程实践。轻松写出正确的异常安全的代码需要许多支持的机制。因此，为了避免需要读者理解全部的调用图，异常代码要隔离出写入持久状态变成“提交”阶段的逻辑。这样既有好处，也有开销（也许你不得不混淆代码和隔离提交）。允许异常会使我们为那些不值得的地方花费开销。
* 允许异常会为二进制程序增加数据，（轻微的）增加编译时间，也有可能增加地址空间的压力。
* 异常会鼓励开发者在不适当的地方抛出异常，在不安全的时候从异常恢复。比如，错误的用户输入不应改引起异常抛出。如果要描述这些限制，我们需要让这篇风格指南变得更长。

总结 ： 
''''''''

表面上看，使用异常的利大于弊，尤其在新的项目中。然而，对于已有的代码，引入异常会对依赖的代码有影响。如果新项目允许异常的话，把它集成在没有异常的代码会是一个大的问题。由于 Google 大部分的 C++ 代码不准备处理异常，引入带有异常的新代码相对来说比较困难。

考虑到 Google 的现有代码不允许异常，在现有代码中使用异常的开销比在新项目中使用异常的开销大的多，转换过程会很慢并且容易出错。我们也不相信异常的取代方案，比如错误代码和断言，引入了很大的负担。

我们不使用异常的建议不是基于哲学和道德理由，而是基于实践。因为我们喜欢用 Google 的开源项目，如果项目中用到了异常就很困难了，所以我们也建议在 Google 的开源项目中不使用异常。如果我们把这些东西推倒重来显然也不现实。

对于 Windows 代码来说，这条规则是个例外（没有双关的意思）。
...
RTTI 允许程序员在运行时决定 C++ 类的对象。这是通过 typeid 或者 dynamic_cast 来完成的。

优点 ：
'''''''''''

取代 RTTI 需要在疑问中修改或者重新设计类的层次。有时这些修改是不可行或者不受欢迎的，尤其在广泛使用或者成熟的代码中。

RTTI 在一些单元测试中会很有作用。比如，在测试工厂类的时候很有永，因为需要测试确定新创建的对象有预期的动态类型。在管理对象和它们的模拟关系时很有用。

当考虑多个抽象对象的时候，RTTI 很有用。比如：
...
在运行时查询一个对象的类型通常意味着设计错误。在运行时才能知道对象的类型经常暗示着你的对象层次设计有缺陷。

滥用 RTTI 使代码变得难以维护，会导致整个代码都有基于类型的决策树或者 switch 语句，当做改变的时候这些都是要被检查的。

总结 ：
''''''''''''

RTTI 有合法的用途，但是容易造成滥用，所以当使用它的时候要格外小心。在单元测试中可以随便使用，但是在其它代码中尽量避免使用。尤其在新代码中，在使用 RTTI 之前要三思。如果你认为需要写出基于对象的类有不同行为的代码，考虑下面的取代方法来检查类型：

*  当需要依赖特定的子类类型来执行不同代码的时候，虚方法是一种更好的选择，因为它把对应的工作放到对象中去了。
* 如果工作在对象之外的代码完成，考虑一个双重分发的解决方案，比如 Visitor 设计模式，允许对象外的工具通过内建类型系统确定类的类型。

当程序的逻辑保证一个基类的实例实际上是一个子类的实例，那么 ``dynamic_cast`` 可以在这个对象上随便用。在这种情况下经常可以使用 ``static_cast`` 代替。

基于类型的据测树强烈预示着你的代码运行在错误的轨道上。
...
当额外的子类被加到类层次中的时候这样的代码通常会失败。同时，当一个子类的属性改变了，很难发现并修改所有影响的代码段。

不要动手去实现一个类似 RTTI 的方案，我们反对 RTTI，也反对带有类型标签的类似类层次的方案。同时，这种方案掩盖了你的真实意图。
